...
GEPInstruction::GEPInstruction(Operand *dst,Operand *base,Operand *baseOffset,std::vector<Operand*> &dimsValAddr, SymbolEntry*arraySe, BasicBlock *insert_bb): Instruction(GEP, insert_bb)
{
    operands.push_back(dst);
    operands.push_back(base);
    operands.push_back(baseOffset);
    dst->setDef(this);
    base->setDef(this);
...
{
    std::string dst,base,arrayType,baseType,baseOffset;
    dst = operands[0]->toStr();
    base=operands[1]->toStr();
    baseType=operands[1]->getEntry()->getType()->toStr();
    baseOffset=operands[2]->toStr();
    arrayType = arraySe->getType()->toStr();
    fprintf(yyout, "  %s = getelementptr %s, %s %s, i32 %s", dst.c_str(), arrayType.c_str(),baseType.c_str(),base.c_str(),baseOffset.c_str());
    int j=operands.size();
    if(j>3)fprintf(yyout,", ");
    for(int i=3;i<j;i++)
    {
        fprintf(yyout,"i32 %s",operands[i]->getEntry()->toStr().c_str());
        if(i!=j-1)
            fprintf(yyout,", ");
    }
    fprintf(yyout,"\n");
...
void AllocaInstruction::genMachineCode(AsmBuilder *builder)
{
    ...
    if(se->getType()->isArray())
    {
        if(((ArrayType*)se->getType())->dimsVal[0]==-1)//这个数组是函数参数的话特殊处理
        {
            ...
            //((ArrayType*)se->getType())->paramOffset=dynamic_cast<TemporarySymbolEntry *>(operands[0]->getEntry())->getOffset();
            // printf("%d ",((ArrayType*)se->getType())->paramOffset);
            // printf("222 ");
            return;
        }
        int elementNum=1;
...
void GEPInstruction::genMachineCode(AsmBuilder *builder)
{
    ...
    {
        ArrayType* at=(ArrayType*)arraySe;
        auto addr=genMachineOperand(operands[0]);
        //int paramOffset=((IdentifierSymbolEntry*)arraySe)->paramNo*4+4;
        //auto stackOffset=genMachineImm(-paramOffset);
        auto stackOffset=genMachineImm(((TemporarySymbolEntry*)operands[1]->getEntry())->getOffset());
        auto fp=genMachineReg(11);
        cur_inst=new LoadMInstruction(cur_block,addr,fp,stackOffset);
        cur_block->InsertInst(cur_inst);

        cur_inst=new BinaryMInstruction(cur_block,BinaryMInstruction::ADD,addr,addr,offsetSum);
        cur_block->InsertInst(cur_inst);
    }
